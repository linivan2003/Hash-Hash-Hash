...
struct hash_table_v1 {
	...
	pthread_mutex_t mutex; // protect entire hash table
};
...
struct hash_table_v1 *hash_table_v1_create()
{
	...
	pthread_mutex_init(&hash_table->mutex, NULL);  //initalize mutex
...
void hash_table_v1_add_entry(struct hash_table_v1 *hash_table,
                             ...
                             uint32_t value)
{
	pthread_mutex_lock(&hash_table->mutex); // Lock before access
	struct hash_table_entry *hash_table_entry = get_hash_table_entry(hash_table, key);
	struct list_head *list_head = &hash_table_entry->list_head;
	struct list_entry *list_entry = get_list_entry(hash_table, key, list_head);
	...
	pthread_mutex_unlock(&hash_table->mutex); // Unlock 
}
...
{
	 pthread_mutex_destroy(&hash_table->mutex); //cleanup
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		struct hash_table_entry *entry = &hash_table->entries[i];
