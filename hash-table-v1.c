...
struct hash_table_v1 *hash_table_v1_create()
{
	...
	int result = pthread_mutex_init(&hash_table->mutex, NULL);
	if (result != 0)
	{
		exit(result);
	}  //initalize mutex
...
void hash_table_v1_add_entry(struct hash_table_v1 *hash_table,
                             ...
                             uint32_t value)
{
	int result = pthread_mutex_lock(&hash_table->mutex);
	if (result != 0)
	{
		exit(result);
	} // Lock before access
	...
	result = pthread_mutex_unlock(&hash_table->mutex); // Unlock 
	if (result != 0)
	{
		exit(result);
	}
...
{
	int result = pthread_mutex_destroy(&hash_table->mutex);
	if (result != 0)
	{
		exit(result);
	} //cleanup
